...
public class ManaLiquidizer {
	...
	private static void attachBeCapabilities(AttachCapabilitiesEvent<BlockEntity> event) {
		...
		if (be instanceof ManaLiquidizerTile container) {
			event.addCapability(prefix("wandable"), CapabilityUtil.makeProvider(BotaniaForgeCapabilities.WANDABLE, (IWandable) be));
			event.addCapability(prefix("mana_receiver"), CapabilityUtil.makeProvider(BotaniaForgeCapabilities.MANA_RECEIVER, (IManaReceiver) be));
			event.addCapability(prefix("spark_attachable"), CapabilityUtil.makeProvider(BotaniaForgeCapabilities.SPARK_ATTACHABLE, (ISparkAttachable) be));
		}
	}
