...
public class ManaLiquidizerBlockEntity extends BotaniaBlockEntity implements IFluidTank, ManaPool, KeyLocked, SparkAttachable, ThrottledPacket, Wandable {
	private int mana;
	...
	public ManaLiquidizerBlockEntity(BlockPos pos, BlockState state) {
		...
		tank = new FluidTank(FluidType.BUCKET_VOLUME * 10, (fluid) -> fluid.getFluid().isSame(fluidActual)); 
	}
	...
	public static void clientTick(Level level, BlockPos worldPosition, BlockState state, ManaLiquidizerBlockEntity tile) {
		tile.initManaCapAndNetwork();
	}

	public static void serverTick(Level level, BlockPos worldPosition, BlockState state, ManaLiquidizerBlockEntity tile) {
		tile.initManaCapAndNetwork();
	...
	public static class WandHud implements WandHUD {
		private final ManaLiquidizerBlockEntity pool;

		public WandHud(ManaLiquidizerBlockEntity pool) {
			this.pool = pool;
		}
	...
	public boolean onUsedByWand(Player player, ItemStack stack, Direction side) {
		if (player == null || player.isShiftKeyDown()) {
			...
			if (player != null && level.isClientSide) {
				player.displayClientMessage(Component.translatable(mode.text), true);
			}
		}
		return true;
	}
